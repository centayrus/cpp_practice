...
// подается байт, идет проверка, что вызов не первый раз, 
// если же первый раз, то вызываются по очереди FinalizeRepeats и AddCharToBlock
    void PutChar(char c) {
...
// добавляется в block байт в позицию block_size_
    void AddCharToBlock(char c) {
        block[block_size_++] = c;
        // если размер блока превышен, то 
...
inline EncodingResult EncodeRLE(std::string src_name, std::string dst_name) {
    ...
    ifstream in(src_name, std::ios::binary);
    if (!in) {
        return {false, 0, 0};
    }
