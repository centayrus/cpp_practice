...
bool PreprocessImpl(const path &in_file, const path &out_file, const path &cur_file, const vector<path> &include_directories) {
    smatch m;
    string str;
    ...
    while (getline(in, str)) {
        ...
        if (regex_match(str, m, includ_file)) {
            ...
            if (!local_path.empty()) {
                PreprocessImpl(parent / inc_path, out_file, in_file, include_directories);
                continue;
            } else {
                path vector_path = VectorDirSearch(inc_path, include_directories);
                if (!vector_path.empty()) {
                    PreprocessImpl(vector_path, out_file, in_file, include_directories);
                    continue;
                }
            }
...
bool PreprocessImpl(const path &in_file, const path &out_file, const vector<path> &include_directories) {
    ifstream in(in_file);
    if (!in) {
        return false;
    }
    return PreprocessImpl(in_file, out_file, in_file, include_directories);
}
