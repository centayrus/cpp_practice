...
public:
    ...
    char GetPixel(Point p) const noexcept {
        /* Реализуйте метод самостоятельно. */
        if (p.y >= size_.height || p.x >= size_.width || p.y < 0 || p.x < 0) {
            return ' ';
        }
        int tile_num = GetTileIndex(p);
        int t_y = p.y % Tile::SIZE;
        int t_x = p.x % Tile::SIZE;
        return tiles_[tile_num]->GetPixel({t_x, t_y});
    }
    ...
    void SetPixel(Point p, char color) {
        ...
        if (!(p.y >= size_.height || p.x >= size_.width || p.y < 0 || p.x < 0)) {
            ...
            int t_y = p.y % Tile::SIZE;
            int t_x = p.x % Tile::SIZE;
            tiles_[tile_num].Write()->SetPixel({t_x, t_y}, color);
        }
    }
