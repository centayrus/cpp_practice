...
public:
    ...
    Witch &operator=(Witch &witch) {
     //   auto tmp_name_ = witch.name_;
       // auto tmp_cat_ = witch.cat_;
        auto tmp = witch;
        name_.swap(tmp.name_);
        cat_.swap(tmp.cat_);
        return *this;
    }

    Witch(const Witch &witch) {
        cat_ = std::make_unique<Cat>(*witch.cat_);
        name_ = witch.name_;
    }
