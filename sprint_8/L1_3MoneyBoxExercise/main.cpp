...
public:
    ...
    void PushCoin(int64_t value) {
        int idx = getIdxOfNominal(value);
        assert(idx < nominals_.size())++ counts_[idx];
    }
...
private:
    ...
    int getIdxOfNominal(const int64_t nominals) {
        for (int i = 0; i < nominals_.size(); ++i) {
            if (nominals_[i] == nominals) {
                return i;
            }
        }
        return nominals_.size();
    }
};
