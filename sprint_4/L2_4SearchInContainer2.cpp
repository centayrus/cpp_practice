...
pair<RandomIt, RandomIt> FindStartsWith(RandomIt range_begin, RandomIt range_end, char prefix) {
    auto first = std::lower_bound(range_begin, range_end, string(1, prefix));
    char next_prefix = static_cast<char>(++prefix);
    auto last = std::lower_bound(range_begin, range_end, string(1, next_prefix));
    return {first, last};
}
