...
public:
    ...
    double Evaluate() const override {
        ...
        switch (type_) {
        case Add:
            result = (*lhs_).Evaluate() + (*rhs_).Evaluate();
            break;
        case Subtract:
            result = (*lhs_).Evaluate() - (*rhs_).Evaluate();
            break;
...
public:
    ...
    double Evaluate() const override {
        ...
        switch (type_) {
        case UnaryPlus:
            result = (*operand_).Evaluate();
            break;
        case UnaryMinus:
            result = -1 * (*operand_).Evaluate();
            break;
        }
        return result;
    }
...
public:
    explicit CellExpr(const Position* cell)
         {
            // здесь добавляем проверку на выход за пределы диапазона
            // если не ок - кидаем FormulaError::Category::Ref
            // иначе:
            cell_ = cell;
    }
