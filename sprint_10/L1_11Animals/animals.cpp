...
void DrawAnimals(const std::vector<const Animal *> &animals, ostream &out) {
    for (const auto *animal : animals) {
        if (const Drawable *draw_animal = dynamic_cast<const Drawable *>(animal)) {
            draw_animal->Draw(out);
        }
    }
}
...
void TalkToAnimals(const std::vector<const Animal *> animals, ostream &out) {
    for (const auto *animal : animals) {
        if (const Speakable *speak_animal = dynamic_cast<const Speakable *>(animal)) {
            speak_animal->Speak(out);
        }
    }
}
