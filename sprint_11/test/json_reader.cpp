...
std::tuple<std::string_view, geo::Coordinates, std::map<std::string_view, int>> JsonReader::FillStop(const json::Dict& request_map) const {
    ...
    for (auto& [stop_name1, dist] : distances) {
        stop_distances.emplace(stop_name1, dist.AsInt());
    }
    ...
    if (request_map.at("underlayer_color").IsString()) render_settings.underlayer_color = request_map.at("underlayer_color").AsString();
        ...
        if (underlayer_color.size() == 3) {
            render_settings.underlayer_color = svg::Rgb(static_cast<uint8_t>(underlayer_color[0].AsInt()),
             static_cast<uint8_t>(underlayer_color[1].AsInt()), static_cast<uint8_t>(underlayer_color[2].AsInt()));
        }
        else if (underlayer_color.size() == 4) {
            render_settings.underlayer_color = svg::Rgba(
                static_cast<uint8_t>(underlayer_color[0].AsInt()), static_cast<uint8_t>(underlayer_color[1].AsInt()),
                 static_cast<uint8_t>(underlayer_color[2].AsInt()), underlayer_color[3].AsDouble());
    ...
    for (const auto& color_element : color_palette) {
        if (color_element.IsString()) render_settings.color_palette.push_back(color_element.AsString());
            ...
            if (color_type.size() == 3) {
                ...
                static_cast<uint8_t>(color_type[1].AsInt()), static_cast<uint8_t>(color_type[2].AsInt())));
            }
            else if (color_type.size() == 4) {
                render_settings.color_palette.push_back(svg::Rgba(static_cast<uint8_t>(color_type[0].AsInt()), 
                static_cast<uint8_t>(color_type[1].AsInt()), static_cast<uint8_t>(color_type[2].AsInt()), color_type[3].AsDouble()));
