...
Builder& Builder::Value(Node::Value value) {
    ...
    if (top_node->IsDict()) {
        ...
        auto [pos, _] = static_cast<Dict>(dict).emplace(std::move(key_.value()), Node{});
        key_ = std::nullopt;
        top_node = &pos->second;
        top_node = std::move(value);
    }
    else if (top_node->IsArray()) {
        auto& array = std::get<Array>(top_node->GetValue());
        static_cast<Array>(array).emplace_back(GetNode(value));
        top_node = &array.back();
    }
...
Builder& Builder::StartDict() {
    ...
    if (top_node->IsDict()) {
        ...
        auto& array = std::get<Array>(top_node->GetValue());
        static_cast<Array>(array).emplace_back(Dict());
...
Builder& Builder::StartArray() {
    ...
    if (top_node->IsDict()) {
        ...
        auto& dict = std::get<Dict>(top_node->GetValue());
        auto [pos, _] = static_cast<Dict>(dict).emplace(std::move(key_.value()), Array());
        key_ = std::nullopt;
        nodes_stack_.emplace_back(&pos->second);
    }
    else if (top_node->IsArray()) {
        auto& array = std::get<Array>(top_node->GetValue());
        static_cast<Array>(array).emplace_back(Array());
        nodes_stack_.emplace_back(&array.back());
    }
    else if (top_node->IsNull()) {
